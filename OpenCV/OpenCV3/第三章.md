...
来源：《OpenCV3编程入门》（毛星云等，电子工业出版社，2015）；书页 63–81（PDF 80–98）

## 本章总览

这一章把 HighGUI 从「能弹出窗口」讲到「能交互」。章首导读回顾：HighGUI 是高层图形界面模块，管媒体输入输出、视频采集、图像 / 视频编解码，以及图形交互。

第 1 章见过的 `VideoCapture` 也属于它，但本章不展开视频类，只把日常交互三件事讲清楚：图怎么读、怎么显示、怎么存；滑动条怎么挂到窗口上；鼠标事件怎么回调。

学完应能回答：

- `imread` 的 flags 怎么选，彩色为什么是 BGR
- 非 8 位图直接 `imshow` 为什么看起来不对
- `namedWindow` 什么时候必须显式调用
- 滑动条和鼠标各自把回调接到哪里
...
`Mat` 本章只点到：默认尺寸为 0；可以指定初始尺寸和初值。书中举例：`cv::Mat pic(320, 640, cv::Scalar(100));`。日常最短用法仍是 `Mat srcImage = imread("dota.jpg");`。

🔑 关键速记：HighGUI 管读显存和窗口交互；图像容器从 `IplImage*` 换成自己管内存的 `Mat`。

#### Mat 构造与类型码

`Mat` 是 OpenCV 2.0 起的主图像容器，内存自己管，替代要手动 `release` 的 `IplImage*`。常用完整构造是 `Mat(rows, cols, type, Scalar)`，type 不能省。

| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| 方法一构造 | 按行、列、类型、初值造图 | `Mat(rows, cols, type, Scalar)` | 本章 `pic(320, 640, Scalar(100))` 没写 type，按第 4 章是不完整写法，不是另一种合法三参数构造 |
| 类型码 | 标明深度和通道 | `CV_[位数][S/U][前缀]C[通道数]`，如 `CV_8UC3` | `Mat` 前两个数是 rows/cols，`Size` 是 width/height |
| 拷贝 | 浅拷贝 vs 深拷贝 | 赋值只拷头；独立副本用 `clone()` / `copyTo()` | `create()` 只分配、不填初值 |

👉 完整深度讲解见第4章。

🔑 关键速记：方法一须含 type；本章 `pic(320, 640, Scalar(100))` 是不完整写法。
...
冲突的 flag 取较小那个数。例如 `COLOR | ANYCOLOR` 仍按三通道彩色加载。想尽量无损、接近源图，书给的组合是 `ANYDEPTH | ANYCOLOR`，即 `2 | 4`。
...
🔑 关键速记：>0 彩色、=0 灰度、<0 带 Alpha；冲突取较小值；尽量无损用 `2|4`。
...
窗口名字当身份证，图本身按 `Mat` 递进去即可。

| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| `imshow` | 把图显示到指定窗口 | `winname`：窗口名。`mat`：类型写成 `InputArray`，实际当 `Mat` | 窗口若按默认 `WINDOW_AUTOSIZE`（OpenCV2 写作 `CV_WINDOW_AUTOSIZE`）创建，图按原尺寸显示；否则缩放到窗口里 |
...
![imshow 按深度缩放：8U 原样；16U/32S 直接显示常全黑；32F 按 [0,1]×255，超出易全白。](./assets/chapter03_imshow_depth.png)
...
🔑 关键速记：8U 原样显示；16U/32S 会 ÷256，32F 按 `[0,1]`×255；先挂滑动条必须先 `namedWindow`。
...
| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| `imwrite` | 把图写到文件 | `filename`：目标文件名。`img`：要保存的图，一般是 `Mat`。`params`：按格式成对填写的可选参数，默认空向量 | 必须带扩展名；能写的类型与 `imread` 能读的一致 |
...
🔑 关键速记：必须带扩展名；JPEG 质量默认 95，PNG 压缩默认 3；3.x 宏去掉 `CV_`。

### 综合示例在干什么（3.1.9）

示例 16 把读、显、叠 logo、存盘串起来。混合细节后文再讲，本章只当预览。

输入 → 现象：

1. `imread("girl.jpg")` → 窗口 1 显示原图。
2. 再读 `dota.jpg`（`flags=199`，即彩色）和 `dota_logo.jpg`。
3. 用 `Rect` 或 `Range` 在大图上圈一块和 logo 一样大的 ROI，再 `addWeighted` 叠上去（书中权重 0.5 / 0.3）→ 融合窗口。
4. `imwrite("由 imwrite 生成的图片.jpg", image)` → 磁盘多一张 jpg。
...
`Rect`/`Range` 已按第 4 章回填。`addWeighted` 完整参数见下。
...
🔑 关键速记：综合例是读图 → 圈 ROI → 加权叠 logo → 存 jpg；权重预览是 0.5 / 0.3。

#### addWeighted

`addWeighted` 按像素做两图加权混合。公式：`dst = src1*alpha + src2*beta + gamma`。两图必须同尺寸、同通道；`alpha + beta` 不必等于 1。

| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| `addWeighted(src1, alpha, src2, beta, gamma, dst, dtype=-1)` | 线性混合 | `dtype=-1` 表示输出深度跟 `src1`。本章预览与第 5 章 ROI 例都是 0.5 / 0.3 | `src2` 必须与 `src1` 同尺寸同通道。多通道时各通道独立算。输出深度不要用 `CV_32S`。整图混合时两图类型、宽高必须一样 |

👉 完整深度讲解见第5章。

🔑 关键速记：两图必须同尺寸同通道；`dtype=-1` 跟 src1；不要用 `CV_32S` 当输出深度。
...
| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| `createTrackbar` | 在已有窗口上挂滑动条 | `trackbarname`：条名。`winname`：须和 `namedWindow` 的名字一致。`value`：指向当前位置的 `int*`，初值就是滑块起点。`count`：最大值；最小值固定为 0。`onChange`：位置一变就调用，默认 0。`userdata`：传给回调的自定义指针，默认 0 | 回调原型必须是 `void XXXX(int, void*)`：第一个是当前位置，第二个是用户数据。传空则只改 `value`、不回调。`value` 若已是全局变量，`userdata` 可以不管 |

回调不是在 `main` 里直接喊，而是滑块动了由系统来喊。最短调用形态（书中对比度例子）：

```cpp
createTrackbar("对比度：", "【效果图窗口】", &g_nContrastValue, 300, on_Change);
```

示例 17 用滑动条做两图线性混合：滑块 0–100，`alpha = 当前位置 / 100`，`beta = 1 - alpha`，再 `addWeighted` 刷新。`namedWindow(WINDOW_NAME, 1)` 后创建滑条，并手动调一次回调，这样初始位置（书中 70）也能先画出来。图 3.6–3.8 分别是透明值 70 / 100 / 0 的效果。
...
🔑 关键速记：先有窗口再挂条；`count` 是最大值、最小固定 0；没有现成按钮，用 0–1 滑条冒充开关。
...
| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| `setMouseCallback` | 把鼠标回调挂到指定窗口 | `winname`：要监听的窗口名。`onMouse`：鼠标事件回调名。`userdata`：传给回调的用户指针，默认 0 | 章末清单写成 `SetMouseCallback`（首字母大写），正文原型是小写；C++ 接口以正文为准 |
...
| `x`, `y` | 指针位置，是图像坐标系，不是窗口坐标系 |
| `flags` | `EVENT_FLAG` 的组合 |
| `param` | 经 `setMouseCallback` 传进来的用户指针 |

OpenCV2 这些常量可以加 `CV_` 前缀（如 `CV_EVENT_MOUSEMOVE`）。【信息缺口：本章未给出完整 `EVENT_*` / `EVENT_FLAG_*` 枚举表，只举例三种鼠标事件。】

示例 18 的交互逻辑：黑底 `600×800`、`CV_8UC3` 画布；`setMouseCallback` 把源图指针传进回调。主循环里先拷到临时图，若正在拖就画临时矩形，`imshow` 刷新；`waitKey(10) == 27` 则 ESC 退出。
...
自定义 `DrawRectangle` 内部调用 `rectangle(img, box.tl(), box.br(), Scalar(...))`，颜色用 `RNG` 随机。
...
🔑 关键速记：`x,y` 是图像坐标；从右下往左上拖宽高为负，松手必须先归一化。

#### rectangle

`rectangle` 在图上画矩形。第 4 章仍未单列官方完整原型，参数从示例 20 还原。对角两点即可；`tl()` / `br()` 是 `Rect` 的左上、右下角。

| 名称 | 功能 | 主要参数 | 约束&坑点 |
|---|---|---|---|
| `rectangle(img, pt1, pt2, color, thickness, lineType)` | 画矩形 | 例中颜色 `Scalar(0, 255, 255)` 为黄（BGR），`lineType = 8` 为 8 连通 | `thickness = -1` 表示实心（与 `circle` 相同约定）。线型对照表见第 8 章 8.1.2 表 8.3 |

👉 完整深度讲解见第4章。

🔑 关键速记：对角两点即可；`thickness=-1` 实心，`lineType=8` 是连通方式不是线宽。
...
🔑 关键速记：静图常用 `waitKey(0)`；循环里短延时，返回 27 当作 ESC。
...
- **`addWeighted` / `Mat` 构造 / `rectangle`**：三者均已按后文回填（见上）。`addWeighted` 以第 5 章原型为准，不要只按本章预览权重当唯一用法。
