...
【第4章回填】完整构造与类型码见 `第四章.md`。对照第 4 章方法一，常用构造是 `Mat(rows, cols, type, Scalar)`，**type 不能省**。本章这行没写 type，按第 4 章应视为不完整写法，不是另一种合法三参数构造。类型码模板：`CV_[位数][S/U][前缀]C[通道数]`，如 `CV_8UC3`。赋值只拷头；独立副本用 `clone()` / `copyTo()`。`create()` 只分配、不填初值。`Mat` 前两个数是 rows/cols，`Size` 是 width/height。
...
【第5章回填】完整讲解见 `第五章.md` 5.2.3。原型：

`addWeighted(src1, alpha, src2, beta, gamma, dst, dtype=-1)`

公式：`dst = src1*alpha + src2*beta + gamma`。`src2` 必须与 `src1` **同尺寸、同通道数**；`dtype=-1` 表示输出深度跟 `src1`。多通道时各通道独立算。**输出深度不要用 `CV_32S`**，可能溢出或算错。`alpha + beta` 不必等于 1（本章预览与第 5 章 ROI 例都是 0.5 / 0.3）。整图混合时两图类型、宽高必须一样。
...
【第4章回填】第 4 章仍未单列 `rectangle` 官方完整原型，从示例 20 可还原的用法：`rectangle(img, pt1, pt2, color, thickness, lineType)`。对角两点即可；`thickness = -1` 表示实心（与 `circle` 相同约定）；例中颜色 `Scalar(0, 255, 255)` 为黄（BGR），`lineType = 8` 为 8 连通。线型对照表见第 8 章 8.1.2 表 8.3。`tl()` / `br()` 是 `Rect` 的左上、右下角。
