...
来源：《OpenCV3编程入门》（毛星云等，电子工业出版社，2015）；书页 85–106（PDF 102–123）

## 本章总览

这一章是第二部分「初探 core 组件」的第一站。章首导读列出四件事：`Mat` 怎么用、控制台怎么格式化打印、常用数据结构有哪些、基本图形怎么画。
...
- 类型码 `CV_8UC3` 怎么拆
- `Scalar(a,b,c)` 哪个是蓝
- `cvtColor` 的 `CV_` 和 `COLOR_` 差在哪
- `thickness = -1` 为什么能画出实心
...
屏幕上看见的是图，设备里存的是每个点的数值。一张图就是一张像素矩阵。OpenCV 的本职工作，就是处理、改写这些矩阵。

🔑 关键速记：图在机器里是像素矩阵，OpenCV 的本职是改这些矩阵。
...
复制大图很贵，所以 OpenCV 用引用计数：每个 `Mat` 有自己的头，但多份对象可以指向同一块像素。赋值、拷贝构造只复制头和指针，不复制像素。
...
![Mat 内存模型：浅拷贝多份头共享同一块像素；clone/copyTo 深拷贝才有独立像素。](./assets/chapter04_mat_copy.png)
...
🔑 关键速记：赋值和拷贝构造只拷头；独立像素必须 `clone` / `copyTo`，改 ROI 就是改原图。
...
🔑 关键速记：默认通道顺序是 BGR；8 位无符号常见范围是 0–255。
...
🔑 关键速记：`CV_8UC3` 是 8 位无符号、3 通道；不要把 `8U` 当成 8 通道。
...
🔑 关键速记：七种造法里 `create()` 不填初值；空白画布用 `zeros`；第 3 章缺 type 的写法对照本章不完整。
...
🔑 关键速记：默认可 `<<`；另外四种风格靠 `format`，2.x 写字符串，3.x 写 `Formatter::FMT_*`。
...
🔑 关键速记：`Point2f` / `Point3f` 和 `vector` 都能 `<<`；`vector<float>` 要先包成 `Mat`。
...
🔑 关键速记：`Point` ≡ `Point2i` ≡ `Point_<int>`；浮点用 `Point2f`。
...
🔑 关键速记：`Scalar(a,b,c)` 按 BGR；`(0,0,255)` 是红，`(0,255,255)` 是黄。
...
🔑 关键速记：`Size` 先宽后高，`Mat` 构造先行后列，矩形时最容易对调。
...
🔑 关键速记：`&` 是交、`|` 是外接并集；`+ point` 平移，`+ size` 缩放。
...
🔑 关键速记：3.x 用 `COLOR_*`，2.x 用 `CV_*`；`COLOR_GRAY2BGR` 是灰度→三通道，不是转灰度。
...
🔑 关键速记：4.2.6 只点名；`Range::all()` 对照 MATLAB 的冒号，完整绘图原型不在这一节。

### 基本图形：五个函数 + 两张示意

4.3 用来画的五个函数：`line`、`ellipse`、`rectangle`、`circle`、`fillPoly`。例程改编自官方样例，自定义几个包装函数，画出化学原子示意和组合图。窗口边长宏：`WINDOW_WIDTH 600`。

本章没有单独列出这五个函数的完整官方原型，参数从包装函数和 `main` 里还原。线型 `lineType` 的完整含义书指向第 8 章 8.1.2 的表 8.3。
...
表 8.3（8.1.2 `drawContours` 的 `lineType`）可选线型：`8`（默认）为 8 连通线；`4` 为 4 连通线；`LINE_AA` 为抗锯齿线（OpenCV2 写 `CV_AA`）。线宽 `thickness` 是另一回事：负数或 `FILLED`（2.x 写 `CV_FILLED`）表示填实，不是线型。
...
🔑 关键速记：五件套无完整官方原型，从示例还原；`thickness=-1` 填实，`lineType=8` 是 8 连通。
