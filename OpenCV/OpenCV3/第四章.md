...
【第8章回填】表 8.3（8.1.2 `drawContours` 的 `lineType`）可选线型：`8`（默认）为 8 连通线；`4` 为 4 连通线；`LINE_AA` 为抗锯齿线（OpenCV2 写 `CV_AA`）。线宽 `thickness` 是另一回事：负数或 `FILLED`（2.x 写 `CV_FILLED`）表示填实，不是线型。
