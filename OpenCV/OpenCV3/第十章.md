...
前面几章在找边、收轮廓、数直方图。这一章换到「兴趣点」：图里哪些位置一挪就变、以后还能再找到。书把这种位置叫**角点**（corner），也常说成特征点检测。导读给的日常定义是两条边的交点；更严一点：某个小邻域里，出现了两个不同方向的边界。实际算法找的往往是梯度、极值这类数学特征，不一定是墙上那个物理尖角。
...
- 什么是角点
- 角点检测的概念
- 用 OpenCV 做 Harris 角点检测
- 用 OpenCV 做 Shi-Tomasi 角点检测
- 亚像素级角点检测
...
函数本身**不画圈**。它只交出一张和原图一样大的分数图。要看见角点，还得自己阈值或扫局部极大。书说后面常接 `threshold()`。

示例 85 流程：灰度读 `1.jpg` → `cornerHarris(..., 2, 3, 0.01)` → `threshold(..., 0.00001, 255, THRESH_BINARY)` → 并排看原图和二值白点。清真寺那组图里，白点堆在穹顶、尖塔和云的高对比处。阈值写得很小，因为原始响应本来就接近 0。

示例 86（10.1.5）加了滑动条，三个窗口：原图、带条的彩色结果、灰度结果。流程：
...
滑动条变量 `thresh` 初值 30、上限 175；真正比较的是 `thresh + 80`。图 10.4 / 10.5 是阈值 13，图 10.6 / 10.7 是 47：条越小圈越多，条越大只剩更「硬」的角。回调注释有一处写成 `on_HoughLines()`，函数名其实是 `on_CornerHarris`。
...
书称这个函数常用来给「按点跟踪」当初始点。

示例 87 流程：读 `1.jpg` → `COLOR_BGR2GRAY` → 滑动条「最大角点数」0～500（初值 33）→ 回调里 `qualityLevel=0.01`、`minDistance=10`、`blockSize=3`、`mask=Mat()`、`useHarrisDetector=false`、`k=0.04` → `goodFeaturesToTrack` → `cout` 打印本次点数 → 每个点画半径 4、颜色随机的圆。控制台标题写的是本书第 87 例、OpenCV 3.0.0-beta；点数随条变成 33 / 133 / 233 / 333。图 10.9 条在 33 时只有穹顶、月亮附近几颗点；图 10.10 条拉到 500，轮廓上密成一片。
...
3. `cornerSubPix(g_grayImage, corners, winSize, zeroZone, criteria)`，再 `cout` 打出带小数的坐标

控制台会出现 `精确角点坐标[110] (426.347, 589.625)` 这种数，整数检测给不出小数。图 10.13 条在 133、图 10.15 条在 301，圈仍然画在建筑棱角上，但坐标已经不是整格。
...
小结还点名：自己做角点函数时可以用 `cornerMinEigenVal` 和 `minMaxLoc`。【信息缺口：这两个函数本章没有原型和参数表。】选点标准要看具体应用；精度要求高时再用 `cornerSubPix`。
...
- **Harris vs Shi-Tomasi**：都看同一个邻域矩阵 \(M\)。Harris 用 \(\det M - k(\mathrm{tr} M)^2\)；Shi-Tomasi 看**较小特征值**够不够大。`goodFeaturesToTrack` 默认走后者；`useHarrisDetector=true` 才切回 Harris，那时 `k` 才有意义。
...
- 示例 85–88：Harris 二值、Harris 滑动条画圈、Shi-Tomasi 限点数、亚像素打印小数。综合例只提炼流程，不写可运行工程。
