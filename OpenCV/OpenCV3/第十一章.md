...
GFTT 就是第10章的 `goodFeaturesToTrack`：在灰度图上按较小特征值筛强角点，直接给出坐标列表，可限最多点数、质量门槛和最小间距。Harris 看同一套邻域矩阵 \(M\)，但用 \(\det M-k(\mathrm{tr} M)^2\) 出响应图，还要自己扫峰。`goodFeaturesToTrack` 默认走 Shi-Tomasi；`useHarrisDetector=true` 才切回 Harris。
...
🔑 关键速记：本章实现按 OpenCV 2.4.9；SIFT/SURF 在 nonfree，OpenCV3 主库书称已迁到 `xfeatures2d`。
...
🔑 关键速记：SURF 图尺寸不变只放大核；26 邻域抑制；60° 扇形定方向；\(20s\) 窗得 64 维。
...
从 `FeatureDetector` 继承，两套重载：单图和多图。
...
单图例都没传 mask；多图这套本章示例未用。

【信息缺口：`SURF` / `SurfFeatureDetector` 完整构造函数参数表本章未列。示例只写了 `SurfFeatureDetector detector(minHessian)`，阈值分别用过 400、700、300、80。】

`minHessian` 越小点越多，越大越「硬」。检测例 400、提取例 700、FLANN 静态例 300、摄像头 SURF 例 80，没有全书统一阈值。

🔑 关键速记：三个类名是 typedef 别名；`minHessian` 示例用过 400 / 700 / 300 / 80，完整构造表本章未列。

### 把点画出来：`drawKeypoints`（11.1.4）

在源图上把已检出的关键点画到输出图。默认每个点随机色。
...
`Scalar::all(-1)` 是随机色不是黑。书把 `keypoints` 写成「输出参数」，实际是已检出的输入点列表。

`DrawMatchesFlags`（11.1.4 / 11.2.1 共用）取值如下。
...
示例 11.1.6 关键调用：两张图、`minHessian = 400`、`detect`、再 `drawKeypoints(..., Scalar::all(-1), DrawMatchesFlags::DEFAULT)`。DEFAULT 像撒了一层针尖；RICH 能看出这个点「管多大一块、朝哪」（图 11.8–11.13）。

![SIFT/SURF 用带尺度方向的圆示意关键点；ORB 点更密、描述子是二进制串。](./assets/chapter11_sift_orb_keypoints.png)

*图注：SIFT/SURF 用带尺度方向的圆示意关键点；ORB 点更密、描述子是二进制串。*

🔑 关键速记：`-1` 是随机色；`DEFAULT` 只画中心，`DRAW_RICH_KEYPOINTS` 带大小和方向。
...
`angle` 负值表示不用方向；`response` 用来排序强弱。

🔑 关键速记：关键点带着坐标、尺度、方向和响应，不只是一个 \((x,y)\)。

### SURF 特征提取：给点算指纹，再暴力对（11.2）

检测只告诉你「这里有点」。提取 / 描述是给每个点算一段向量。书强调：尺度决定窗口多大，理想描述子不怕光照和小透视；比较时常用欧氏距离。
...
knn 用第二套（每点一组）。筛好匹配后常用 `NOT_DRAW_SINGLE_POINTS`。颜色 `-1` 仍是随机色。

🔑 关键速记：扁平 `vector<DMatch>` 对 `match`；分组 `vector<vector<DMatch>>` 对 knn。
...
示例 11.2.3 关键调用：`SurfFeatureDetector detector(700)` 检出点（阈值改成 700，比检测例的 400 更严）→ `SurfDescriptorExtractor extractor; extractor.compute(图, 点, 描述子Mat)` → `matcher.match(descriptors1, descriptors2, matches)` → `drawMatches` 画线。

输入输出现象：两盒包装并排，细线把对应点连起来（图 11.14）。

🔑 关键速记：暴力是每个对每个；SURF 默认 64 维，用 L2；提取例 `minHessian = 700`。
...
第一套当场传入训练描述子；第二套用事先 `add` / `train` 好的库。
...
被 mask 挡掉的查询不会写入 `matches`，条数可能少于查询点数。

🔑 关键速记：`match` 每点只给第一名；库可以当场传，也可以先 `add`/`train`。

#### 静态两图 + 按最小距离筛（示例 11.3.3）

关键调用：`minHessian = 300`，`SURF` 既当 detector 又当 extractor → `FlannBasedMatcher matcher; matcher.match(...)` → 先扫一遍 `matches[i].distance`，记下最大、最小距离（例：Max 0.557617，Min 0.047603）。

好匹配：`distance < 2 * min_dist`。

书评：也可以改用 `radiusMatch`【信息缺口：`radiusMatch` 本章无原型】。

再 `drawMatches(..., DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS)`，控制台按 `queryIdx` / `trainIdx` 打印「特征点1 — 特征点2」。

🔑 关键速记：静态 FLANN 例用 `minHessian = 300`、好匹配门槛 `2 × min_dist`；`radiusMatch` 本章无原型。

#### 摄像头实时：SURF + FLANN + Lowe（示例 11.3.4）
...
输入输出现象：对着无关物体，线很少（图 11.19）；对着同一本书，线又多又齐（图 11.20）。帧率大约 **3.2～4.4 FPS**，书说偏低。计时仍是 `(getTickCount()-t0)/getTickFrequency()` 的倒数当帧率。

【信息缺口：`knnMatch` / `add` / `train` 本章只有调用，无完整官方原型。】

🔑 关键速记：摄像头 SURF 例 `minHessian = 80`，Lowe 0.6；`knnMatch` / `add` / `train` 无完整原型。

#### SIFT + 暴力匹配（示例 11.3.5 与表 11.1）
...
输入输出现象：帧率大约 **2.6～3.0**。书说在相近代码下，这种匹配帧率比 SURF 平均大约低 4.0 FPS。无关物体几乎没线，对上同一本书则线很齐（图注写「暴力匹配」）。

【信息缺口：`SiftFeatureDetector` / `SiftDescriptorExtractor` / `SIFT` 完整构造参数本章未列。】

🔑 关键速记：SIFT 128 维、DOG；综合例走暴力 + Lowe 0.6；完整构造本章未列。

### 寻找已知物体：匹配之后再框出来（11.4）
...
![两图好匹配连线示意；筛距离或 Lowe 后再可选单应性框物体；拼图右半边画框时 x 要加左图 cols。](./assets/chapter11_feature_match_lines.png)

*图注：两图好匹配连线示意；筛距离或 Lowe 后再可选单应性框物体；拼图右半边画框时 x 要加左图 cols。*
...
头文件：`calib3d.hpp`。示例用 `findHomography(obj, scene, CV_RANSAC)`。`mask` 传入的旧值会被忽略。

🔑 关键速记：点为 `CV_32FC2` 或 `vector<Point2f>`；示例走 `CV_RANSAC`；`mask` 只当输出。

#### `perspectiveTransform`（11.4.2）

用 \(3\times3\) 或 \(4\times4\) 矩阵变换点集。输出必须与输入同尺寸同类型。
...
2D 用 3×3，就是上面的 \(H\)。`dst` 须与 `src` 同尺寸同类型。

🔑 关键速记：把物体四角乘 \(H\) 变到场景坐标；输出类型必须跟输入一致。

#### 示例 11.4.3 关键调用

`minHessian = 400` → SURF 检点、算描述子 → FLANN `match` → 按距离筛好匹配：`distance < 3 * min_dist`（注意：这里是 3 倍，11.3.3 是 2 倍）→ 从 `good_matches` 取出 `queryIdx` / `trainIdx` 对应的 `pt` → `H = findHomography(obj, scene, CV_RANSAC)` → 物体四角 `(0,0)`、`(cols,0)`、`(cols,rows)`、`(0,rows)` 做 `perspectiveTransform` → 在 `img_matches` 上画四条线。
...
🔑 关键速记：找物体例好匹配用 `3 × min_dist`；拼图画框的 \(x\) 要加左图 `cols`。
...
🔑 关键速记：三个名字等价；ORB 补旋转和噪声，尺度未从根上解决；完整构造本章未列。
...
输入输出现象：帧率大约 **23～30**（图 11.29 出现 23.46、27.32、29.75、30.77），比 SURF/SIFT 实时例高一个数量级。无关物体几乎没线（图 11.30）；对上目标书则线多且整齐（图 11.31）。

🔑 关键速记：ORB 用 Hamming + `LshIndexParams(12,20,2)` + Lowe 0.6；三参数含义与 `VideoCapture::set` 宏本章未解释。
...
- **GFTT / Harris vs 本章特征链路**：第10章停在角点坐标；本章继续算描述子再匹配。不要把 `goodFeaturesToTrack` 的点列表直接当成 SURF/ORB 描述子。👉 完整深度讲解见第10章。
...
- 数字不要跨例改写：`minHessian` 为 400 / 700 / 300 / 80；好匹配为 `2 × min_dist` 与 `3 × min_dist`；Lowe 为 0.6。
- 本章示例按 OpenCV 2.4.9 写。OpenCV3 要把 SIFT/SURF/ORB 等到 `xfeatures2d` 里找。综合例只提炼输入输出现象与关键调用，不写可运行工程。
