...
【第7章回填】完整说明见 `第七章.md` 的 Canny 节。这里只补第一章缺的部分，不展开 Sobel / 霍夫。

| 名称 | 第7章补全 |
|---|---|
| `Canny(image, edges, threshold1, threshold2, apertureSize=3, L2gradient=false)` | 六个参数。日常常只传前四个或前五个。`image` 必须 8 位单通道；`edges` 与输入同尺寸同类型。 |
| 双阈值 | `threshold1` 低、`threshold2` 高。较小者用来把弱边连上，较大者先圈出强边。夹在中间的点只有连着强边才留下。推荐高低比 **2:1～3:1**。第一章 `3,9` 正好是 3:1；综合例把高阈值写成低阈值 ×3。 |
| `apertureSize` | 内部 Sobel 孔径，默认 3。第一章两处最后一个 `3` 就是它。 |
| `L2gradient` | 是否用更精确的 L2 范数算梯度幅值，默认 `false`。第一章调用没传这个参数。 |
| OpenCV3 | 不要对彩色图直接 `Canny`（OpenCV2 简写本章已不适用）。标准顺序：灰度 → `blur` → `Canny`，与第一章流程一致。 |
